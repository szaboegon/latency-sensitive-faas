...
def handler(event):
    ...
    cv2.rectangle(
        image,
        (json_data["cropped_coords"]["startX"], startY),
        (json_data["cropped_coords"]["endX"], json_data["cropped_coords"]["endY"]),
        COLORS[index],
        2,
    )
    ...
    cv2.putText(
        image,
        label,
        (json_data["cropped_coords"]["startX"], y),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.5,
        COLORS[index],
        2,
    )
    # Return image as base64 in JSON
    return {"result": image_to_base64(image)}
