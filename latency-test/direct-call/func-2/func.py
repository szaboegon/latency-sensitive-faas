...
import os
import requests
from dummy2 import handler as dummy2
...
if 'tracer' not in globals():
    tracer = tracing.instrument_app("func-2-direct")
...
def send(event_out, headers):
    requests.post("http://func-3-direct.application.svc.cluster.local", data=event_out, headers=headers)
